...
void configure_i2c(){
    ...
    P1->SEL0 |= BIT6 | BIT7;                    // configure pins for I2C
...
void trigger_sample_mpl() {
    // get and set OST low
    uint8_t setting = read_register(CTRL_REG1);
    setting &= ~(1 << 1);                   // clear OST bit
    write_register(CTRL_REG1, setting);

    // set OST high again
    setting = read_register(CTRL_REG1);
...
uint8_t read_temp_c() {
    ...
    while(status & 0x08 == 0) {
        status = read_register(OUT_T_MSB);
    }
    temp_c = read_register(OUT_T_MSB);
    return temp_c;
}
...
void set_mode_standby() {
    uint8_t setting = read_register(CTRL_REG1);  // read setting
    setting &= ~(1 << 0);                       // set standby bit
    write_register(CTRL_REG1, setting);          // write to reg
}
...
void enable_event_flags() {
  write_register(PT_DATA_CFG, 0x07); // Enable all three pressure and temp event flags
}
